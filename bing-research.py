...
# -----------------------------
# JSON file to persist excluded domains
# -----------------------------
...
# Page config
# -----------------------------
st.set_page_config(page_title="Ultimate Bing SEO Tool", layout="wide")
st.title("🔎 Ultimate Bing SEO Research Tool")

# -----------------------------
# Load excluded domains into session
# -----------------------------
...
# -----------------------------
st.markdown("---")
st.header("⚙️ Excluded Domains Manager")

# Compact Add / Remove layout
...
# -----------------------------
st.markdown("---")
st.header("📌 Find Keywords & Top Pages by Domain")
...
if run_analysis:
    ...
    params = {"engine":"bing","q":query,"api_key":SERPAPI_KEY,"num":result_count}
...
# -----------------------------
st.markdown("---")
st.header("🏆 Bing Affiliate Competitor Finder")
...
if find_comps:
    params = {"engine":"bing","q":comp_keyword,"api_key":SERPAPI_KEY,"num":50}
    search = GoogleSearch(params)
    results = search.get_dict()
    organic = results.get("organic_results", [])
